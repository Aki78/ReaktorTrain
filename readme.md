...
The server at pythonanywhere.com seems to be unstable (even with the paid prescription), and crashes for no reason sometimes so there is a chance the data won't update. (Currently the service not not working properly anymore. The update doesn't happen. I have no control over it since it is a cloud server, I just have to wait until they fix it.)
